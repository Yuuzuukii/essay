...
\jtitle[LLMマルチエージェント型質問応答のための弁証法議論]%	% 和文題目（内容梗概／目次用）
	{LLMマルチエージェント型質問応答のための弁証法議論}	% 和文題目
\etitle{A Dialectical Argumentation for LLM Multi-Agent Question Answering}	% 英文題目
\jauthor{増尾 柚希}				% 和文著者名
\eauthor{Yuzuki MASUO}			% 英文著者名
\supervisor{村上 陽平 教授}		% 指導教官名
\department{2025年度（秋学期）卒業研究3（CH）}

\date{令和8年1月30日}				% 提出年月日

\begin{document}
\maketitle					% 「とびら」の出力
...
続いて4章では評価として，用いる先行研究の対話例を説明し，対話例を用いてプロトコルの正当性を検証する．
5章で検証結果に基づく考察を述べ，最後に6章で本研究のまとめと将来的な展望を述べる．
...
そこで，本研究では，記号論理に基づく弁証法議論の構造的厳密性と，
...
使用するモデルはGPT-4o-miniである．
プロトコルにおいて議論の終了状態に遷移した時，議論制御コンポーネントは議論を終了し，最終回答を出力する．

次節では，本研究の主題である弁証法プロトコルの詳細設計について説明する．
...
\subsection{評価の目的}\label{subsec-experiment-purpose}

本章では，前章で定義した弁証法プロトコルを実際の問題設定に適用し，提案手法の妥当性を検証する．
具体的には，提案プロトコルによって生成される議論過程および止揚論証が，既存の計算的弁証法モデルに基づく形式例と，構造および意味の両面において対応しているかを確認することを目的とする．
...
\subsection{実験設定}\label{subsec-experiment-setup}

\subsubsection{比較対象}\label{subsubsec-eval-data}

本研究では，提案する弁証法プロトコルの妥当性を検証するための比較対象として，Kido，Kuriharaによって提案されたカメラ購買問題に関する対話例（文献[1] 第6章）を用いる．
本項では，評価の基準となる形式例を明確にするため，文献中で用いられている問題設定および対話過程を示す．
...
\[
\begin{aligned}
\mathrm{Arg}_{1}^{5} &= [\,userFriendly(c),\ battery(c, \mathrm{long}),\ camera(c), userFriendly(c)\\
...
\subsubsection{実験条件}\label{subsubsec-experiment-condition}

題材には，前項で示したKido，Kurihara[1] 第6章のカメラ購買問題を採用した．
...
\subsection{実験結果}\label{subsec-experiment-result}

\subsubsection{対話結果}
以下では，主張，反論，再反論の各段階で生成された論証を，対話ログの画像として提示する．（図\ref{fig:dialogue1}，図\ref{fig:dialogue2}）．
それぞれの変数名は，前章で定義したものと同じである．

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\linewidth]{dialogue1.png}
  \caption{対話ログ1}
  \label{fig:dialogue1}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\linewidth]{dialogue2.png}
  \caption{対話ログ2}
  \label{fig:dialogue2}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{対話ログ3（統合フェーズによる止揚論証の生成）}
  \label{fig:synthesis}
\end{figure}
...
図\ref{fig:synthesis}は，性質化および統合を経て得られた合意核に基づき，最終的な止揚論証が構築される過程を示したものである．


図\ref{fig:dialogue1}，図\ref{fig:dialogue2}，図\ref{fig:synthesis}に示すように，本研究のプロトコルでは，多ターンの議論を通じて対立点が段階的に明確化され，
最終的に性質化および統合を経て，単なる折衷ではない止揚論証が構築されていることが確認できる．


\subsection{結果の分析}\label{subsec-eval-result}

本節では，4.3.1項で示した形式例（文献[1] 第6章）と，
図\ref{fig:dialogue1}，図\ref{fig:dialogue2}および図\ref{fig:synthesis}に示した本研究の実行結果を比較し，
...
統合の具体的な流れについては既存研究の対話例の中には示されていなかったため，対応関係を示すことはできないが，warrantの入力から双方の購買条件となる
性質をそれぞれ抽出し，それらを包含する条件を新たに構築することで，同様の結論"Choose camera c,..."が導かれている．
したがってこの点においても形式例の$\mathrm{Arg}_{1}^{5}$と構造的に一致していると言える．

以上より，本研究の実行結果は，形式例における主張構築，反論の流れと，反論不能による主張エージェントの交代，および統合という一連の推論過程を，論理構造を保ちながら再現できており，
...
論証を構造化スキーマとして表現し，rebutおよびundercutに基づく反論フェーズと，統合フェーズからなる弁証法プロトコルを設計・実装した点に本研究の特徴がある．
